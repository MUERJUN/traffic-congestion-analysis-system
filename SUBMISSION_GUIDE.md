...
脚本会自动识别项目根目录，在当前目录和上一级寻找其余分包，尽可能解压并恢复原始数据、清洗数据和随机森林模型，校验分片后安装 `requirements.txt` 与 `requirements-dashboard.txt`，最后启动Streamlit。已安装依赖时可执行 `python run_dashboard.py --skip-install`。
