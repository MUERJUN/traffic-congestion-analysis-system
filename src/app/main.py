...
def hero(title: str, subtitle: str) -> None:
    st.markdown(f'<div class="hero"><div class="eyebrow">METR-LA · HISTORICAL REPLAY</div><h1>{title}</h1><p>{subtitle}</p></div>', unsafe_allow_html=True)
...
def introduction() -> None:
    hero("城市道路拥堵风险分析", "从数据质量、规律分析到未来 30 分钟风险解释的完整证据链")
    cards = (("207 个观测点", "METR-LA 高速公路传感器，不将传感器直接等同于完整城市道路。"), ("5 分钟粒度", "连续历史速度观测，用于分析周期规律、趋势与历史回放案例。"), ("未来 30 分钟", "正式模型结果只读取 Track A 固化的版本化产物。"))
    for col, (title, body) in zip(st.columns(3), cards):
        col.markdown(f'<div class="card"><h3>{title}</h3>{body}</div>', unsafe_allow_html=True)
    st.subheader("分析闭环")
    st.markdown("**数据审计** → **探索分析** → **风险预测** → **模型解释** → **业务建议**")
    st.subheader("使用指引")
